# Opensource-FISH
# FISH: 인공지능 기반 보이스피싱 및 스미싱 방지 시스템
> "단순 화면 캡처 업로드"만으로 피싱 문맥과 사칭 URL을 실시간 탐지하는 사회적 약자 배려형 AI 보안 솔루션

본 프로젝트는 지능화되는 타겟형 스미싱 범죄를 실시간 단계에서 차단하기 위해 비전 엔진(EasyOCR)의 이미지 전처리, 편집 거리(Levenshtein Distance) 추적, 그리고 **Dual AI Engine (TF-IDF MLP/ KoBERT)** 각각의 가중치를 통해 피싱 여부를 검증하는 시스템입니다.
...
┌───────────────────────────┼───────────────────────────┐
▼                           ▼                           ▼
[독립 모듈 1: VD-Verifier]   [독립 모듈 2: TF-IDF + MLP]  [독립 모듈 3: KoBERT Analyzer]
 (도메인 주소 정밀 검증)      (본문 키워드 통계 분류)      (한국어 특화 문맥 분류)
  - CSV 직접 매칭 (0순위)      - Char N-gram (2,4) 벡터화   - skt/kobert-base-v1 기반
  - 화이트리스트 직접 대조      - Sequential MLP 가중치 추론  - 트랜스포머 파인튜닝 가동
  - Levenshtein 편집 거리      - Sigmoid 기반 피싱 확률     - 문맥 흐름 노이즈 탐지
  - 로지스틱 회귀 동적 분류
        │                           │                           │
        ▼                           ▼                           ▼
 [Gradio 시연 화면 ①]        [Gradio 시연 화면 ②]        [Gradio 시연 화면 ③]
"편집거리 기반 URI 탐지 모듈" "TF-IDF+MLP기반 차단 시스템" "KoBERT 기반 실시간 차단 시스템"
